...
import { Vendor, Restaurant, MenuItemTemplate, Order, User, UserRole, RestaurantPermissions, PermissionSchedule, ActiveHours, OrderManagementConfig, BoardTemplate, MenuTemplate, MenuSection, Ingredient, ProductionSpace, Promotion, PromotionType } from '../types';
import { fileToBase64, ChartIcon, SettingsIcon, MenuBookIcon, PlusIcon, EditIcon, TrashIcon, ClipboardListIcon, ChefHatIcon, ShoppingBagIcon, CheckCircleIcon, HourglassIcon, FlagCheckeredIcon, PackageIcon, SparklesIcon, XCircleIcon } from './Shared';
...
const Analytics: React.FC<{restaurant: Restaurant, orders: Order[], users: User[], ingredients: Ingredient[], menuItems: MenuItemTemplate[]}> = ({ restaurant, orders, users, ingredients, menuItems }) => {
    ...
    const { 
        ...
        // Detailed Order Times for Analysis
        ...
        <div className="fixed inset-0 bg-black bg-opacity-60 z-[60] flex justify-center items-center p-4">
            <form onSubmit={handleSubmit} className="bg-white rounded-lg shadow-xl w-full max-w-lg flex flex-col">
                ...
                <div className="p-4 flex-grow">
                    <div><label className="block text-sm font-medium text-gray-800">{t('name')}</label><input value={formData.name || ''} onChange={e => setFormData(p => ({...p, name: e.target.value}))} className="w-full mt-1 p-2 border rounded-md text-gray-800" required /></div>
                </div>
                <div className="p-4 border-t flex justify-end space-x-3">
                    <button type="button" onClick={onClose} className="px-4 py-2 bg-gray-200 rounded-lg">{t('cancel')}</button>
...
const MenuItemTemplateModal: React.FC<{
    ...
    // FIX: Crash prevented by ensuring productionSpaceIds is initialized as array even if undefined in item prop
    ...
    const smartEstimatedTime = useMemo(() => {
        if (isNewItem || !item.id) return null;
        const relevantOrders = orders.filter(o => o.status === 'completed' && o.items.some(i => i.id === item.id) && o.completionTime);
        if (relevantOrders.length < 3) return null; 
        const totalTime = relevantOrders.reduce((sum, o) => sum + (o.completionTime || 0), 0);
        const avgTime = Math.round(totalTime / relevantOrders.length);
        if (Math.abs(avgTime - (formData.prepTime || 10)) > 2) return avgTime;
        return null;
    }, [orders, item.id, formData.prepTime, isNewItem]);
                ...
                <div className="p-6 flex-grow overflow-y-auto space-y-4">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input id="name" name="name" value={formData.name} onChange={handleChange} className="w-full mt-1 p-2 border rounded-md bg-gray-100 text-gray-800" required />
                        </div>
                         <div>
                            <label className="block text-sm font-medium text-gray-800">{t('assigned_spaces')}</label>
                            <div className="mt-1 p-2 border rounded-md bg-gray-100 max-h-32 overflow-y-auto">
                    ...
                    <div>
                        ...
                        <textarea id="description" name="description" value={formData.description} onChange={handleChange} rows={3} className="w-full mt-1 p-2 border rounded-md bg-gray-100 text-gray-800" required />
                    </div>
                    <div className="grid grid-cols-3 gap-4">
                         <div>
                            <label htmlFor="price" className="block text-sm font-medium text-gray-800">{t('price')}</label>
                            <input id="price" name="price" type="number" step="0.01" value={formData.price} onChange={handleChange} className="w-full mt-1 p-2 border rounded-md bg-gray-100 text-gray-800" required />
                        </div>
                        <div>
                            <label htmlFor="prepTime" className="block text-sm font-medium text-gray-800">{t('prep_time')}</label>
                            <input id="prepTime" name="prepTime" type="number" step="1" value={formData.prepTime} onChange={handleChange} className="w-full mt-1 p-2 border rounded-md bg-gray-100 text-gray-800" required />
                            {smartEstimatedTime && (
                                <div className="mt-2 text-xs p-2 bg-blue-50 text-blue-800 rounded border border-blue-200 flex justify-between items-center animate-pulse">
                                    <span>History suggests: <strong>{smartEstimatedTime} mins</strong></span>
                                    <button type="button" onClick={() => setFormData(p => ({...p, prepTime: smartEstimatedTime}))} className="text-blue-600 underline font-bold">Update</button>
                                </div>
                            )}
                        </div>
                        <div>
                            <label htmlFor="capacityUsage" className="block text-sm font-medium text-gray-800">{t('capacity_load')}</label>
                            <input id="capacityUsage" name="capacityUsage" type="number" step="0.1" value={formData.capacityUsage} onChange={handleChange} className="w-full mt-1 p-2 border rounded-md bg-gray-100 text-gray-800" required />
                        </div>
                    </div>
                    
                    {/* Separate Fulfillment Checkbox */}
                    ...
                    <div>
                        ...
                        <div className="flex items-center space-x-2 mt-1">
                            <input name="imageUrl" value={formData.imageUrl?.startsWith('data:') ? '' : formData.imageUrl} onChange={handleChange} placeholder={t('paste_image_url')} className="w-full p-2 border rounded-md bg-gray-100 text-gray-800" />
                     ...
                     <div>
                        ...
                        <div className="p-2 border rounded-md mt-1">
                            ...
                            <div className="flex space-x-2 items-end flex-wrap">
                                <select value={selectedIngredient} onChange={(e: React.ChangeEvent<HTMLSelectElement>) => setSelectedIngredient(e.target.value)} className="flex-grow p-2 border rounded-md bg-gray-100 text-gray-800 min-w-[150px]">
                                    <option value="">-- {t('select_ingredient')} --</option>
                                    {ingredients.map(ing => <option key={ing.id} value={ing.id}>{ing.name}</option>)}
                                </select>
                                <input type="number" min="0.01" step="0.01" value={ingredientQuantity} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setIngredientQuantity(parseFloat(e.target.value))} className="w-20 p-2 border rounded-md bg-gray-100 text-gray-800" placeholder="Qty" />
                                <label className="flex items-center p-2 bg-gray-50 rounded border">
                                    <input type="checkbox" checked={isMandatoryInput} onChange={(e) => setIsMandatoryInput(e.target.checked)} className="mr-2"/>
                                    <span className="text-xs">{t('mandatory')}</span>
                                </label>
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-60 z-[60] flex justify-center items-center p-4">
            <form onSubmit={handleSubmit} className="bg-white rounded-lg shadow-xl w-full max-w-3xl max-h-[90vh] flex flex-col">
                ...
                <div className="p-6 flex-grow overflow-y-auto space-y-4">
                    <input value={name} onChange={e => setName(e.target.value)} className="w-full border p-2 rounded text-gray-800" placeholder={t('menu_name')} required />
                    {sections.map(section => (
                         <div key={section.id} className="border p-4 rounded bg-gray-50 mb-2">
                             <input value={section.title} onChange={e => handleUpdateSection(section.id, 'title', e.target.value)} className="border-b font-bold bg-transparent mb-2 text-gray-800"/>
                    ...
                    <button type="button" onClick={handleAddSection} className="text-primary">+ {t('add_section')}</button>
                </div>
...
const InventoryManager: React.FC<{
    ...
    return (
        <div className="space-y-6">
            <div className="flex justify-between items-center">
                ...
                <div className="flex space-x-2">
                     <input 
                        ...
                        className="p-1.5 border rounded bg-white text-gray-800 placeholder-gray-500" // Fixed text color visibility
                     />
            ...
            {editingIng && (
                <div className="fixed inset-0 bg-black bg-opacity-60 z-[60] flex justify-center items-center p-4">
                    <form onSubmit={handleSave} className="bg-white rounded-lg shadow-xl p-6 w-full max-w-md space-y-4 max-h-[90vh] overflow-y-auto">
                        <h3 className="text-xl font-bold text-secondary">{editingIng.id ? t('edit') : t('add')} {t('ingredients')}</h3>
                        <div className="grid grid-cols-2 gap-4">
                             <div><label className="text-sm font-medium">{t('name')}</label><input value={editingIng.name || ''} onChange={e => setEditingIng({...editingIng, name: e.target.value})} className="w-full border rounded p-2 text-gray-800" required /></div>
                             <div><label className="text-sm font-medium">{t('sku')}</label><input value={editingIng.sku || ''} onChange={e => setEditingIng({...editingIng, sku: e.target.value})} className="w-full border rounded p-2 text-gray-800" /></div>
                        </div>
                         <div>
                            <label className="text-sm font-medium">{t('category')}</label>
                            {/* Changed to Input with Datalist for Custom Categories */}
                            <input 
                                list="categories" 
                        ...
                        <div className="grid grid-cols-2 gap-4">
                             <div><label className="text-sm font-medium">{t('cost_unit')}</label><input type="number" step="0.01" value={editingIng.costPerUnit} onChange={e => setEditingIng({...editingIng, costPerUnit: parseFloat(e.target.value)})} className="w-full border rounded p-2 text-gray-800" required /></div>
                             <div>
                                <label className="text-sm font-medium">{t('unit')}</label>
                                <select value={editingIng.unit} onChange={e => setEditingIng({...editingIng, unit: e.target.value as any})} className="w-full border rounded p-2 text-gray-800">
                        ...
                        <div className="border-t pt-4">
                            <h4 className="font-semibold text-sm mb-2 flex justify-between">
                                ...
                                <button type="button" onClick={addLocation} className="text-primary text-xs">+ {t('add_location')}</button>
                            </h4>
                            <div className="space-y-2 max-h-40 overflow-y-auto bg-gray-50 p-2 rounded">
                                {editingIng.locations?.map((loc, idx) => (
                                    <div key={idx} className="flex space-x-2 items-center">
                                        <input placeholder={t('location_name')} value={loc.name} onChange={e => updateLocation(idx, 'name', e.target.value)} className="flex-grow p-1 text-sm border rounded text-gray-800"/>
                                        <input type="number" placeholder="Qty" value={loc.quantity} onChange={e => updateLocation(idx, 'quantity', parseFloat(e.target.value))} className="w-20 p-1 text-sm border rounded text-gray-800"/>
...
const SpaceManager: React.FC<{
    ...
    return (
        <div className="space-y-4">
            ...
            {editingSpace && (
                 <div className="fixed inset-0 bg-black bg-opacity-60 z-[60] flex justify-center items-center p-4">
                    <form onSubmit={handleSave} className="bg-white rounded-lg shadow-xl p-6 w-full max-w-md space-y-4">
                        <h3 className="text-xl font-bold text-secondary">{editingSpace.id ? t('edit_space') : t('create_space')}</h3>
                        <div><label className="text-sm font-medium">{t('name')}</label><input value={editingSpace.name || ''} onChange={e => setEditingSpace({...editingSpace, name: e.target.value})} className="w-full border rounded p-2 text-gray-800" required placeholder="e.g. Grill Station, Bar, Oven"/></div>
                        <div>
                            <label className="text-sm font-medium">{t('workflow_template')}</label>
                            <select value={editingSpace.boardTemplateId || ''} onChange={e => setEditingSpace({...editingSpace, boardTemplateId: parseInt(e.target.value)})} className="w-full border rounded p-2 text-gray-800" required>
                                <option value="">-- {t('workflow_template')} --</option>
                                {myTemplates.map(t => <option key={t.id} value={t.id}>{t.name}</option>)}
                            </select>
                        </div>
                         <div><label className="text-sm font-medium">{t('total_capacity')}</label><input type="number" min="1" value={editingSpace.concurrentCapacity} onChange={e => setEditingSpace({...editingSpace, concurrentCapacity: parseInt(e.target.value)})} className="w-full border rounded p-2 text-gray-800" required /></div>
...
const OrderManager: React.FC<{
    ...
    useEffect(() => {
        const interval = setInterval(() => setNow(new Date()), 10000); // Live update for timers
    ...
    const relevantOrders = orders.filter(o => o.restaurantId === restaurantId);
    ...
    return (
        <div className="flex space-x-4 overflow-x-auto pb-4 h-[calc(100vh-200px)]">
            {columns.map(column => {
                const columnOrders = relevantOrders.filter(o => column.statusIds.includes(o.status));
                const ColumnIcon = column.icon ? ICON_MAP[column.icon] : null;

                return (
                    <div key={column.id} className="min-w-[320px] rounded-lg flex flex-col max-h-full border border-gray-200 shadow-sm" style={{ backgroundColor: column.columnColor || '#F3F4F6' }}>
                                    ...
                                    >
                                        <div className="flex justify-between items-start mb-2">
                                            ...
                                            <div className="text-right">
                                                <span className="font-bold text-gray-800 block">${order.total.toFixed(2)}</span>
...
const MarketingManager: React.FC<{
    ...
    return (
        <div className="space-y-8 animate-fade-in-down">
            ...
            {isCreating && (
                 <div className="fixed inset-0 bg-black bg-opacity-60 z-[60] flex justify-center items-center p-4">
                    <form onSubmit={handleCreateSubmit} className="bg-white rounded-lg shadow-xl p-6 w-full max-w-md space-y-4">
                        ...
                        <div>
                            ...
                            <input value={newPromo.title} onChange={e => setNewPromo({...newPromo, title: e.target.value})} className="w-full mt-1 p-2 border rounded-md" placeholder="e.g. Summer Sale" required />
                        </div>
                         <div className="grid grid-cols-2 gap-4">
                             <div>
                                <label className="block text-sm font-medium text-gray-800">{t('type')}</label>
                                <select value={newPromo.type} onChange={e => setNewPromo({...newPromo, type: e.target.value as PromotionType})} className="w-full mt-1 p-2 border rounded-md">
                             ...
                             <div>
                                ...
                                <input type="number" value={newPromo.value} onChange={e => setNewPromo({...newPromo, value: parseFloat(e.target.value)})} className="w-full mt-1 p-2 border rounded-md" required />
                             </div>
                        </div>
                         <div>
                        ...
                        <div className="flex justify-end space-x-2 pt-4">
                            <button type="button" onClick={() => setIsCreating(false)} className="px-4 py-2 bg-gray-200 rounded-md">{t('cancel')}</button>
                            <button type="submit" className="px-4 py-2 bg-primary text-white rounded-md">{t('create_promo_btn')}</button>
                        </div>
                    </form>
                </div>
            )}
                        ...
                        <div className="space-y-12">
                            <section>
                                ...
                                <SpaceManager 
                                    ...
                                    onUpdateSpace={props.onUpdatePointOfSale}
                                    onDeleteSpace={props.onDeletePointOfSale}
                                />
                            </section>

                            <section>
                                <h2 className="text-2xl font-bold text-gray-800 mb-4 border-b pb-2">{t('restaurant_details')}</h2>
...
const RestaurantDetailsForm: React.FC<{restaurant: Restaurant, onSave: (r: Restaurant) => void}> = ({ restaurant, onSave }) => {
    ...
    return (
        <div className="bg-white p-6 rounded-lg shadow max-w-2xl">
            ...
            <button onClick={() => onSave({...restaurant, name})} className="bg-primary text-white px-4 py-2 rounded">{t('save_details')}</button>
        </div>
    )
}
            ...
            {staff.map(u => (
                <div key={u.id} className="flex justify-between items-center bg-white p-4 rounded shadow">
                    ...
                    <div className="flex space-x-2">
                        ...
                        <button onClick={() => onDeleteUser(u.id)} className="text-red-500 hover:bg-red-50 p-1 rounded"><TrashIcon className="w-5 h-5"/></button>
                    </div>
                </div>
            ))}
            {isCreating ? (
                <form onSubmit={handleCreate} className="bg-gray-50 p-4 rounded border">
                    <input placeholder={t('name')} className="block w-full mb-2 p-2 border rounded" value={newUser.name} onChange={e => setNewUser({...newUser, name: e.target.value})} required/>
                    <input placeholder={t('username')} className="block w-full mb-2 p-2 border rounded" value={newUser.username} onChange={e => setNewUser({...newUser, username: e.target.value})} required/>
                    <input placeholder={t('password')} type="password" className="block w-full mb-2 p-2 border rounded" value={newUser.password} onChange={e => setNewUser({...newUser, password: e.target.value})} required/>
                    <div className="flex space-x-2">
                        <button type="submit" className="bg-primary text-white px-3 py-1 rounded">{t('create')}</button>
                        <button type="button" onClick={() => setIsCreating(false)} className="bg-gray-300 px-3 py-1 rounded">{t('cancel')}</button>
                    </div>
                </form>
            ) : (
                <button onClick={() => setIsCreating(true)} className="text-primary hover:underline text-sm">+ {t('add_manager')}</button>
            )}
    ...
    return (
        <div className="space-y-4">
            <div className="flex justify-between items-center mb-4">
                ...
                <button onClick={() => setEditingTemplate({})} className="bg-primary text-white px-3 py-1.5 rounded-md flex items-center text-sm">
